  ...
  render() {
    ...
    return (
      <MowContainer
        ...
        title={
          this.state.cart
            ? mowStrings.addressList.deliveryInformation
            : mowStrings.addressList.title
        }>
        {/* show the view if the navigation comes from cart */}
        {/* info header view */}
        {this.state.cart && <MowInfoHeader activeIndex={1} />}
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <TouchableOpacity
              onPress={() => {
                this._handleAddressSelection(index, item);
              }}
              key={index}
              style={[
                categoryStyleWithoutShadow,
