...
export class MowButtonBasic extends React.Component {
  ...
  _setTextStyle() {
    return (this.textStyle = {
      color: this._getSelectedTextColor(),
      marginHorizontal: this.props.stickyIcon ? 10 : 0,
  ...
  _setIconStyle() {
    return (this.iconStyle = {
      color: this._getSelectedTextColor(),
      position: this.props.stickyIcon ? 'relative' : 'absolute',
    });
  }
  ...
  render() {
    ...
    return (
      <MowButtonBase
        {...this.props}
        disabled={loading}
        style={[this.btnStyle, this.props.containerStyle]}>
        {this.state.leftIcon && (
          <FeatherIcon
            style={[
              this.iconStyle,
        ...
        <Text style={[this.textStyle, {...this.props.textStyle}]}>
          {this.props.children}
        </Text>
